...
namespace VirtoCommerce.CustomerExportImportModule.Data.Validation
{
        ...
        where T: CsvMember
    {
        ...
        {
            RuleFor(x => x.Record.OuterId).MaximumLength(128).WithErrorCode(ModuleConstants.ValidationErrors.ExceedingMaxLength);
            ...
            When(x => new[]
                ...
                , () =>
                {
                    RuleFor(x => x.Record.AddressType).IsEnumName(typeof(AddressType)).WithErrorCode(ModuleConstants.ValidationErrors.InvalidValue);

                    RuleFor(x => x.Record.AddressFirstName).MaximumLength(128).WithErrorCode(ModuleConstants.ValidationErrors.ExceedingMaxLength);
                    RuleFor(x => x.Record.AddressLastName).MaximumLength(128).WithErrorCode(ModuleConstants.ValidationErrors.ExceedingMaxLength);

                    RuleFor(x => x.Record.AddressEmail).MaximumLength(64).WithErrorCode(ModuleConstants.ValidationErrors.ExceedingMaxLength);
                    RuleFor(x => x.Record.AddressPhone).MaximumLength(256).WithErrorCode(ModuleConstants.ValidationErrors.ExceedingMaxLength);

                    RuleFor(x => x.Record.AddressLine1).NotEmpty().WithErrorCode(ModuleConstants.ValidationErrors.MissingRequiredColumns);
            ...
            RuleFor(x => x.Record.DynamicProperties).SetValidator(new ImportDynamicPropertiesValidator());
        }
    }
}
